...
- **Scenario writing targets DE 1.57 and 1.58.** Older versions (1.55–1.56)
  read fine but are read-only. Map resizing and arbitrary existing-unit field
  edits are not exposed.
...
AoE2Kit is licensed **LGPL-3.0-only** ([LICENSE](LICENSE)). It is source-first:
public releases ship source and attribution, not stale built artifacts.

It stands on other people's work, credited deliberately rather than in passing.
AGE (Advanced Genie Editor), by Tapsa and contributors (GPL-3.0), informed the
`.dat` authoring semantics, field terminology, and editor-oracle expectations;
`aoe2scenario-parser` and its community shaped the scenario layer. The full
ledger lives in [NOTICE.md](NOTICE.md) and `data/credits.json`:
